...
def preprocess_user_input(user_input):
    ...
    input_tokenizado = nltk.word_tokenize(user_input)
    # Convertir el input del usuario en su palabra raíz, es decir, usar un proceso de stemming
    input_tokenizado_mejorado = get_stem_words(input_tokenizado, ignore_words)
    # Remover palabras duplicadas y ordenar user_input
    input_tokenizado_mejorado = sorted(list(set(input_tokenizado_mejorado)))
    # Códificación de datos de entrada. Crear una bolsa de palabras para user_input
    for palabra in words:
        if palabra in input_tokenizado_mejorado:
